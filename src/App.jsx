...
const NoXcuroApp = () => {
  // VISTAS PRINCIPALES
  const [currentView, setCurrentView] = useState('capture'); // capture, micelio, temporalChat, carousel, archive, simulador
  ...
  // SIMULADOR ESPACIO-TIEMPO: Estado del lienzo de dudas
  const [simNodes, setSimNodes] = useState([
    { id: 'jalo', name: 'El Jalón', label: 'Fuerza Gravitatoria', x: 400, y: 300, mass: 1000, color: '#dc2626', radius: 50, draggable: false },
    { id: 'camino', name: 'Camino', label: 'Línea de Distancia', x: 700, y: 200, color: '#3b82f6', radius: 15, draggable: true },
    { id: 'reloj1', name: 'Reloj 1', label: 'Observador A', x: 200, y: 150, color: '#f59e0b', radius: 25, draggable: true, time: 0, timeDilation: 1 },
    { id: 'reloj2', name: 'Reloj 2', label: 'Observador B', x: 150, y: 450, color: '#10b981', radius: 25, draggable: true, time: 0, timeDilation: 1 }
  ]);
  ...
  // ============================================
  // SIMULADOR ESPACIO-TIEMPO: LÓGICA DE FÍSICA
  // ============================================
  const calcularDistancia = (nodoA, nodoB) => {
    const dx = nodoA.x - nodoB.x;
    const dy = nodoA.y - nodoB.y;
    return Math.sqrt(dx * dx + dy * dy);
  };

  const calcularDilatacionTemporal = (distancia, masa) => {
    // Fórmula simplificada inspirada en relatividad: t' = t * sqrt(1 - 2GM/rc²)
    const G = 6.674;
    const c = 3e8;
    const factor = (2 * G * masa) / (distancia * c * c);
    return Math.sqrt(Math.max(0.5, 1 - factor)); // Mínimo 0.5 para que nunca se pare completamente
  };

  const calcularFuerzaGravedad = (distancia, masa) => {
    const G = 6.674;
    return (G * masa) / (distancia * distancia);
  };

  // Animación del segundero de los relojes
  useEffect(() => {
    if (currentView !== 'simulador') return;
    let animationId;
    ...
    const animate = (currentTime) => {
      ...
      setSimNodes(prev => prev.map(nodo => {
        if (!nodo.id.startsWith('reloj')) return nodo;
        
        const jalo = prev.find(n => n.id === 'jalo');
        if (!jalo) return nodo;
        
        const distancia = calcularDistancia(nodo, jalo);
        const factorTiempo = calcularDilatacionTemporal(distancia, jalo.mass);
        
        // Actualizar ángulo del segundero
        const velocidadBase = 6.28; // 2π radianes por segundo
        const nuevoTiempo = (nodo.time || 0) + delta * velocidadBase * factorTiempo;
        
        // Color basado en la dilatación temporal
        const color = factorTiempo > 0.9 
          ? '#f59e0b' // Ámbar: tiempo casi normal
          : factorTiempo > 0.6 
            ? '#8b5cf6' // Violeta: tiempo moderadamente lento
            : '#dc2626'; // Rojo: tiempo muy lento
        
        return {
          ...nodo,
          time: nuevoTiempo % (2 * Math.PI),
          color: color,
          timeDilation: factorTiempo
        };
      }));
  ...
  // Drag and Drop para nodos del simulador
  const handleSimMouseDown = (e, nodoId) => {
    const nodo = simNodes.find(n => n.id === nodoId);
    if (nodo && nodo.draggable) {
      setDragging({ id: nodoId, offsetX: e.clientX - nodo.x, offsetY: e.clientY - nodo.y });
    }
  };
  ...
  const handleSimMouseMove = (e) => {
    ...
    const scaleX = 1000 / rect.width;  // viewBox width
    const scaleY = 600 / rect.height;  // viewBox height
    
    const newX = Math.max(30, Math.min(970, (e.clientX - rect.left) * scaleX));
    const newY = Math.max(30, Math.min(570, (e.clientY - rect.top) * scaleY));
  ...
  // FUNCIONES: Carrusel
  // ============================================
  // SIMULADOR ESPACIO-TIEMPO: VISTA PRINCIPAL
  // ============================================
  const renderSimulador = () => (
    <div className="max-w-7xl mx-auto p-6 animate-in fade-in duration-700">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 font-serif italic text-sm">
              {translatorMode ? 'Arrastra los nodos para explorar el espacio-tiempo' : 'El universo responde a tu curiosidad'}
            </p>
          </div>
        </div>
        ...
        >
          {translatorMode ? '🔮 Modo Traductor Activo' : '🌍 Modo Nativo'}
        </button>
      </div>

      {/* Canvas del Simulador */}
      <div className="bg-white rounded-[2.5rem] border-2 border-slate-200 shadow-2xl overflow-hidden">
        <div className="relative bg-gradient-to-br from-slate-950 via-purple-950 to-slate-900" style={{ height: '600px' }}>
          <svg
            id="sim-canvas"
          ...
          >
            {/* Fondo estelar */}
            <rect width="1000" height="600" fill="url(#spaceGlow)" />
            
            {/* Estrellas generadas proceduralmente */}
            {Array.from({ length: 50 }).map((_, i) => (
              <circle key={i} cx={Math.random() * 1000} cy={Math.random() * 600} r={Math.random() * 1.5} fill="white" opacity={Math.random() * 0.5 + 0.2}>
                <animate attributeName="opacity" values={`${Math.random()*0.5+0.2};${Math.random()*0.8+0.3};${Math.random()*0.5+0.2}`} dur={`${Math.random() * 3 + 2}s`} repeatCount="indefinite" />
              </circle>
            ))}

            {/* Líneas de conexión elásticas (Camino) */}
            {simNodes.map(nodo => {
              if (nodo.id === 'jalo') return null;
              const jalo = simNodes.find(n => n.id === 'jalo');
              if (!jalo) return null;
              const distancia = calcularDistancia(nodo, jalo);
              const fuerza = calcularFuerzaGravedad(distancia, jalo.mass);
              
              const curvatura = Math.min(fuerza * 5000, 100);
              const midX = (nodo.x + jalo.x) / 2;
              const midY = (nodo.y + jalo.y) / 2 - curvatura;
              const opacidad = Math.max(0.3, 1 - distancia / 500);
              
              return (
                <g key={`line-${nodo.id}`}>
                  <path
                    d={`M ${jalo.x} ${jalo.y} Q ${midX} ${midY} ${nodo.x} ${nodo.y}`}
                    fill="none"
                    stroke="#94a3b8"
                    strokeWidth="2"
                    strokeDasharray="8 4"
                  ...
                  {translatorMode && (
                    <text x={midX} y={midY - 10} textAnchor="middle" fontSize="11" fontWeight="bold" fill="#cbd5e1" className="uppercase tracking-wider">
                      d={Math.round(distancia)}m
                    </text>
                  )}
                </g>
              );
            ...
            {/* NODO: El Jalón (Masa gravitatoria) */}
            {simNodes.filter(n => n.id === 'jalo').map(nodo => {
              const escala = 1 + (nodo.mass / 1000);
              return (
                <g key={nodo.id}>
                  <circle cx={nodo.x} cy={nodo.y} r={nodo.radius * 2} fill="none" stroke="#dc2626" strokeWidth="1" opacity="0.3">
                    <animate attributeName="r" values={`${nodo.radius*1.5};${nodo.radius*2.5};${nodo.radius*1.5}`} dur="3s" repeatCount="indefinite" />
                    <animate attributeName="opacity" values="0.3;0.1;0.3" dur="3s" repeatCount="indefinite" />
                  </circle>
                  <circle cx={nodo.x} cy={nodo.y} r={nodo.radius} fill="#450a0a" stroke="#dc2626" strokeWidth="4" />
                  <circle cx={nodo.x} cy={nodo.y} r={nodo.radius * 0.6} fill="#dc2626">
                    <animate attributeName="r" values={`${nodo.radius*0.5};${nodo.radius*0.7};${nodo.radius*0.5}`} dur="2s" repeatCount="indefinite" />
                  </circle>
                  
                  <text x={nodo.x} y={nodo.y + nodo.radius + 20} textAnchor="middle" fontSize="13" fontWeight="900" fill="#fca5a5" className="uppercase tracking-widest">
                    {translatorMode ? `El Jalón (M=${nodo.mass})` : nodo.name}
                  </text>
                  {translatorMode && (
                    <text x={nodo.x} y={nodo.y + nodo.radius + 38} textAnchor="middle" fontSize="9" fontWeight="bold" fill="#94a3b8">
                      Fuerza Gravitatoria
                    </text>
                  )}
                </g>
            ...
            {/* NODOS: Relojes */}
            {simNodes.filter(n => n.id.startsWith('reloj')).map(nodo => (
              <g key={nodo.id} onMouseDown={(e) => handleSimMouseDown(e, nodo.id)} style={{ cursor: 'grab' }}>
                <circle cx={nodo.x} cy={nodo.y} r={nodo.radius} fill={nodo.color || '#f59e0b'} stroke="#ffffff" strokeWidth="3" opacity="0.9" />
                
                <g transform={`translate(${nodo.x}, ${nodo.y})`}>
                  {Array.from({ length: 12 }).map((_, i) => {
                    const angle = (i * 30) * Math.PI / 180;
                    const x1 = Math.cos(angle) * (nodo.radius - 8);
                    const y1 = Math.sin(angle) * (nodo.radius - 8);
                    const x2 = Math.cos(angle) * (nodo.radius - 3);
                    const y2 = Math.sin(angle) * (nodo.radius - 3);
                    return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="white" strokeWidth="2" />;
                  })}
                  
                  <line
                    x1="0" y1="0"
                    x2={Math.cos((nodo.time || 0) - Math.PI/2) * (nodo.radius - 10)}
                    y2={Math.sin((nodo.time || 0) - Math.PI/2) * (nodo.radius - 10)}
                    stroke="white"
                    strokeWidth="2"
                    strokeLinecap="round"
                  />
                ...
                <text x={nodo.x} y={nodo.y + nodo.radius + 20} textAnchor="middle" fontSize="12" fontWeight="900" fill={nodo.color || '#f59e0b'} className="uppercase tracking-wider">
                  {translatorMode ? `${nodo.name} (${nodo.label})` : nodo.name}
                </text>
                
                {translatorMode && (
                  <text x={nodo.x} y={nodo.y + nodo.radius + 35} textAnchor="middle" fontSize="9" fontWeight="bold" fill="#94a3b8">
                    t'={(nodo.timeDilation || 1).toFixed(3)}
                  </text>
                )}
              </g>
            ))}

            {/* NODO: Camino (Línea de distancia) */}
            {simNodes.filter(n => n.id === 'camino').map(nodo => (
              <g key={nodo.id} onMouseDown={(e) => handleSimMouseDown(e, nodo.id)} style={{ cursor: 'grab' }}>
                <circle cx={nodo.x} cy={nodo.y} r={nodo.radius} fill="#1e40af" stroke="#3b82f6" strokeWidth="3" />
                <path d={`M ${nodo.x - 8} ${nodo.y} L ${nodo.x + 8} ${nodo.y} M ${nodo.x} ${nodo.y - 8} L ${nodo.x} ${nodo.y + 8}`} stroke="white" strokeWidth="2" strokeLinecap="round" />
                
                <text x={nodo.x} y={nodo.y + nodo.radius + 20} textAnchor="middle" fontSize="12" fontWeight="900" fill="#93c5fd" className="uppercase tracking-wider">
                  {translatorMode ? `${nodo.name} (${nodo.label})` : nodo.name}
                </text>
              </g>
            ))}
          ...
          <div className="absolute bottom-6 left-6 right-6 bg-black/60 backdrop-blur-md rounded-2xl p-4 border border-white/10">
            <div className="flex items-start gap-4">
              <div className="text-white/60 text-xs leading-relaxed flex-1">
                ...
                <p>• Arrastra los nodos azules (Camino) y los relojes (verde/naranja)</p>
                <p>• Observa cómo los relojes se ralentizan al acercarse al "Jalón" rojo</p>
                <p>• Activa el Modo Traductor para ver las métricas físicas</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4 mt-6">
        <div className="bg-white rounded-2xl p-6 border-2 border-slate-200 shadow-lg">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Fuerza Gravitatoria</p>
          <p className="text-2xl font-black text-slate-900">
            {(() => {
              const jalo = simNodes.find(n => n.id === 'jalo');
              const camino = simNodes.find(n => n.id === 'camino');
              if (!jalo || !camino) return '0.00';
              const d = calcularDistancia(jalo, camino);
              return calcularFuerzaGravedad(d, jalo.mass).toFixed(2);
            })()}
            <span className="text-sm text-slate-500 ml-2">N</span>
          </p>
        </div>
        
        <div className="bg-white rounded-2xl p-6 border-2 border-slate-200 shadow-lg">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Dilatación Promedio</p>
          <p className="text-2xl font-black text-slate-900">
            {(() => {
              const relojes = simNodes.filter(n => n.id.startsWith('reloj'));
              const jalo = simNodes.find(n => n.id === 'jalo');
              if (!jalo || relojes.length === 0) return '1.000';
              const avg = relojes.reduce((sum, r) => {
                const d = calcularDistancia(r, jalo);
                return sum + calcularDilatacionTemporal(d, jalo.mass);
              }, 0) / relojes.length;
              return avg.toFixed(3);
            })()}
            <span className="text-sm text-slate-500 ml-2">t'/t</span>
          </p>
        </div>
        
        <div className="bg-white rounded-2xl p-6 border-2 border-slate-200 shadow-lg">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Estado del Tiempo</p>
          <p className="text-2xl font-black text-slate-900">
            {(() => {
              const jalo = simNodes.find(n => n.id === 'jalo');
              const reloj1 = simNodes.find(n => n.id === 'reloj1');
              if (!jalo || !reloj1) return 'Normal';
              const d = calcularDistancia(jalo, reloj1);
              const factor = calcularDilatacionTemporal(d, jalo.mass);
              if (factor > 0.9) return 'Normal';
              if (factor > 0.6) return 'Lento';
              return 'Muy Lento';
            })()}
          </p>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-blue-50 font-sans text-slate-900">
      ...
      <main className="py-8 px-4">
        ...
        {currentView === 'simulador' && renderSimulador()}
      </main>
    </div>
  );
};
